...
// herr i am using event listeners to change the background color of the card on mouseover and mouseout events.

let options = document.querySelector(".cards");
options.addEventListener("mouseover", function() {
    options.style.backgroundColor = "lightblue";
});
 

// mouseout is used to remove the background color when the mouse is not over the card.

let removeColor = document.querySelector(".cards");
removeColor.addEventListener("mouseout", function() {
    removeColor.style.backgroundColor = "white";
});

// mouse move is used to change the background color of the card when the mouse is over the card.

window.addEventListener("mousemove", function(event) {
    let x = event.clientX;
    let y = event.clientY;
    options.style.backgroundColor = `rgb(${x % 255}, ${y % 255}, 150)`;
});